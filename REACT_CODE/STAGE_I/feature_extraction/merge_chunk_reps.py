...
def merge_tensors_cambrian(base_dir, output_file_name, batch_size=20, data_per_chunk=3326, num_workers=None,is_normalize=True):
    """
    ...
    if is_normalize:
        logger.info(f"Looking for normalized files")
    else:
        logger.info(f"Looking for unormalized files")
    ...
    for subdir in valid_subdirs:
        ...
        if is_normalize:
            file_suffix = f"chunk_{chunk_number}_all_normalized.pt" 
        else:
            file_suffix=f"chunk_{chunk_number}_all_unnormalized.pt"
        target_file = subdir / file_suffix
        logger.info(f"Checking for {file_suffix} in {subdir}/")
        if target_file.exists():
            logger.info(f"Found file: {target_file}")
            all_files.append(target_file)
            chunk_numbers.append(chunk_number)
        else:
            logger.warning(f"Missing file {file_suffix} in {subdir}/")
